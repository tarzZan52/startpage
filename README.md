...
**🪟 Windows:** Запустите `server/install-service-windows.bat` от имени администратора  
...
<details>
<summary>🪟 Windows команды</summary>

После автоматической установки доступны команды:

**Через командную строку (Win+R → cmd):**
```cmd
# Остановить сервер
schtasks /end /tn "DashboardWebServer"

# Запустить сервер
schtasks /run /tn "DashboardWebServer"

# Проверить статус
schtasks /query /tn "DashboardWebServer"
```

**Полное удаление:**
```cmd
# Запустите в папке server
...
<details>
<summary>🔧 Права администратора (Windows)</summary>

Если скрипт требует права администратора:
1. Закройте командную строку
2. Найдите файл `install-service-windows.bat`
